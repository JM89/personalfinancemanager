...
namespace PersonalFinanceManager.Services
{
    public class UserProfileService : IUserProfileService
    {
        public void CreateUserProfile(UserProfileEditModel model)
        {
            ...
            {
                var dto = AutoMapper.Mapper.Map<PersonalFinanceManager.Api.Contracts.UserProfile.UserProfileDetails>(model);
        ...
        public UserProfileEditModel GetByUserId(string userId)
        {
            ...
            using (var httpClient = new HttpClientExtended())
            {
                var response = httpClient.GetSingle<PersonalFinanceManager.Api.Contracts.UserProfile.UserProfileDetails>($"/UserProfile/GetByUserId/{userId}");
        ...
        public UserProfileEditModel GetById(int id)
        {
            ...
            using (var httpClient = new HttpClientExtended())
            {
                var response = httpClient.GetSingle<PersonalFinanceManager.Api.Contracts.UserProfile.UserProfileDetails>($"/UserProfile/Get/{id}");
                result = AutoMapper.Mapper.Map<UserProfileEditModel>(response);
            }
            return result;
        }
