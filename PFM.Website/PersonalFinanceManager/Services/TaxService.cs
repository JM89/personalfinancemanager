...
namespace PersonalFinanceManager.Services
{
    public class TaxService : ITaxService
    {
        public void CreateTax(TaxEditModel model)
        {
            ...
            {
                var dto = AutoMapper.Mapper.Map<PersonalFinanceManager.Api.Contracts.Tax.TaxDetails>(model);
        ...
        public TaxEditModel GetById(int id)
        {
            ...
            using (var httpClient = new HttpClientExtended())
            {
                var response = httpClient.GetSingle<PersonalFinanceManager.Api.Contracts.Tax.TaxDetails>($"/Tax/Get/{id}");
        ...
        public IList<TaxListModel> GetTaxes(string userId)
        {
            ...
            using (var httpClient = new HttpClientExtended())
            {
                var response = httpClient.GetList<PersonalFinanceManager.Api.Contracts.Tax.TaxList>($"/Tax/GetList/{userId}");
        ...
        public IList<TaxListModel> GetTaxesByType(string currentUser, TaxType incomeTax)
        {
            ...
            using (var httpClient = new HttpClientExtended())
            {
                ...
                var response = httpClient.GetList<PersonalFinanceManager.Api.Contracts.Tax.TaxList>($"/Tax/GetTaxesByType/{currentUser}/{taxTypeId}");
                result = response.Select(AutoMapper.Mapper.Map<TaxListModel>).ToList();
            }
            return result;
        }
