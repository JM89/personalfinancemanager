...
namespace PersonalFinanceManager.Services
{
    public class AtmWithdrawService : IAtmWithdrawService
    {
        public IList<AtmWithdrawListModel> GetAtmWithdrawsByAccountId(int accountId)
        {
            ...
            {
                var response = httpClient.GetList<PersonalFinanceManager.Api.Contracts.AtmWithdraw.AtmWithdrawList>($"/AtmWithdraw/GetList/{accountId}");
        ...
        public void CreateAtmWithdraws(List<AtmWithdrawEditModel> models)
        {
            using (var httpClient = new HttpClientExtended())
            {
                var dto = models.Select(AutoMapper.Mapper.Map<PersonalFinanceManager.Api.Contracts.AtmWithdraw.AtmWithdrawDetails>).ToList();
        ...
        public void CreateAtmWithdraw(AtmWithdrawEditModel model)
        {
            using (var httpClient = new HttpClientExtended())
            {
                var dto = AutoMapper.Mapper.Map<PersonalFinanceManager.Api.Contracts.AtmWithdraw.AtmWithdrawDetails>(model);
        ...
        public AtmWithdrawEditModel GetById(int id)
        {
            ...
            using (var httpClient = new HttpClientExtended())
            {
                var response = httpClient.GetSingle<PersonalFinanceManager.Api.Contracts.AtmWithdraw.AtmWithdrawDetails>($"/AtmWithdraw/Get/{id}");
