...
namespace PersonalFinanceManager.Services
{
    public class BankAccountService: IBankAccountService
    {
        ...
        {
            var response = await _httpClientExtended.GetList<PFM.Api.Contracts.Account.AccountList>($"/BankAccount/GetList/{userId}");
            return response.Select(AutoMapper.Mapper.Map<AccountListModel>).ToList();
        }
        
        public async Task<AccountEditModel> GetById(int id)
        {
            var response = await _httpClientExtended.GetSingle<PFM.Api.Contracts.Account.AccountDetails>($"/BankAccount/Get/{id}");
