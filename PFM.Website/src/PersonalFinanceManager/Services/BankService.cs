...
namespace PersonalFinanceManager.Services
{
    public class BankService : IBankService
    {
        ...
        {
            var response = await _httpClientExtended.GetList<PFM.Bank.Api.Contracts.Bank.BankList>($"/Bank/GetList");
            return response.Select(AutoMapper.Mapper.Map<BankListModel>).ToList();
        }

        public async Task<bool> CreateBank(BankEditModel model)
        {
            var dto = AutoMapper.Mapper.Map<PFM.Bank.Api.Contracts.Bank.BankDetails>(model);
            return await _httpClientExtended.Post($"/Bank/Create", dto);
        }

        public async Task<BankEditModel> GetById(int id)
        {
            var response = await _httpClientExtended.GetSingle<PFM.Bank.Api.Contracts.Bank.BankDetails>($"/Bank/Get/{id}");
